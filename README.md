...
メモ用途では、まず `memo.txt` などを一度保存してから書き始めてください。
保存済みファイルへの加筆・修正は、保存前でも LSP の文書同期でサーバーに届くため、
校正・変換・翻訳の対象になります。操作のたびに保存する必要はありません。

一方、一度も保存していない `Untitled` の新規タブは別扱いです。2026-09-20 に確認した
...
他形式への対応方針とテキスト校正くんとの差異は
[設計方針](docs/architecture.md#対応形式と校正方法)を参照してください。
