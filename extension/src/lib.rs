...
const SERVER_DIST_DIR: &str = "text-tools-server";
const CURRENT_VERSION_FILE: &str = "CURRENT_VERSION";

// `lsp.text-tools.binary` に明示設定が無いときのデフォルトの起動引数。
// 拡張の作業ディレクトリ配下の `text-tools-server/<version>/src/lsp/server.js` を絶対パスで指す。
