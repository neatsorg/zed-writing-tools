...
追記: i18n へ移植した字数カウントの分離と、独立した未保存文書パッチの新設について、
[リポジトリ分離・個人用ビルド手順書](docs/repository-separation-plan.md)を作成した。
取り込み構成の変更は同書を優先する。`zed-scratch-buffers` と `zed-personal-build` を新設する方針で、
後者は GitHub に置く場合も private とする。現段階は調査・手順書のみで、分離や新設は未実施。
...
ユーザーがGUIで動作確認済み（問題なし）。開発順序2は完成範囲を満たした。
