...
当初の暫定対応（後述のパッチで撤回）: `message.range` は変換せず、そのまま UTF-16 オフセットとして使う
...
## UTF-16 位置パッチによるスキップ撤回

文書全体で形態素解析系 7 ルールをスキップする暫定対応を撤回した。
最終的な textlint の診断ではなく、kuromoji 0.1.2 のトークン生成時に位置を修正する。

- `Tokenizer.tokenizeForSentence`: 内部ラティスのコードポイント位置を、その解析入力に
  対する UTF-16 の 1 始まり位置へ変換する。ノード・文の開始位置を加える前なので、
  ルールによる文分割や改行の違いに依存しない。kuromojin のキャッシュには補正済みの
  トークンが入るため、キャッシュ再利用時に二重変換しない。
- `ViterbiBuilder`: 連続する未知語を結合した `key` が通常の JS 文字列になり、長さだけ
  UTF-16 単位になる問題も修正。例えば `😀😀食べれる。` は位置だけでなく後続トークンも
  欠落し得た。未知語の長さを内部ラティスに合わせてコードポイント単位に統一した。
- 辞書・原文・ルールの判定条件は変更しない。正規表現系を含め、textlint の最終 range は
  UTF-16 のまま使う。30000 字超の重いルールを省く既存方針は維持する。

パッチは `patches/` に差分と適用前後の SHA-256 を保存し、バージョンとソースが合致する
場合だけ適用する。通常のインストール・テスト前・起動前と、配布物の依存導入後に適用。
`--ignore-scripts` でインストール後に直接起動する際は `npm run patch:deps` が必要。
校正配布物は適用済みソースとパッチ記録を含み、変換配布物には含めない。

全 7 ルールについて、単一／連続絵文字・𠮷・ZWJ 絵文字・CRLF・空行・結合文字の後の
指摘範囲を固定の期待値で検証し、キャッシュ再利用、正規表現系との併用、LSP の行・文字位置も
検証する。上流依存の更新時は、これらの回帰テストとパッチの必要性を再確認すること。
