...
（経緯は本リポジトリの調査・設計資料を参照）。共通サーバーの実装は共有するが、
...
2026-09-20 時点では、3 拡張の `extension.toml` の対応言語は `Plain Text` のみ。
他形式への対応は今後の作業であり、まだ追加していない。拡張側の
`language_servers.<id>.languages` に言語名を追加することで、利用者に言語ごとの
`settings.json` 追記を求めず対応言語を登録できる
（[Zed の言語サーバー登録仕様](https://zed.dev/docs/extensions/languages#language-servers)）。
利用者が `language_servers` を独自に制限している場合は、その設定も確認する。

校正については、対応言語を増やす際も当面は全文をプレーンテキストとして扱う方針。
Markdown のコードブロックや HTML のタグなども検査対象になることを許容し、
形式別の構文解析・除外処理を対応言語追加の必須条件にはしない。
変換・翻訳も引き続き選択された文字列を処理する。

参考にした VS Code の「テキスト校正くん」は、すべてをプレーンテキストとして
処理しているわけではない。2026-09-20 に確認した
[上流の `src/server.ts`](https://github.com/ics-creative/project-japanese-proofreading/blob/master/src/server.ts)
では、テキスト・Markdown 用の既定プラグインに HTML・LaTeX・Re:VIEW 用プラグインを
加え、対応拡張子を確認し、ファイルパスを `lintText` に渡して形式に応じて解析している。
このプロジェクトの全文プレーンテキスト処理とは区別する。将来、構文部分の指摘が
支障になった場合に形式別プラグインの導入を検討する。

未保存文書では「保存済みファイルの未保存の編集」と「一度も保存していない新規タブ」を
区別する。後者には Zed 側の LSP 登録制約があるため、メモ用の `.txt` を一度保存して
使う回避策を [README](../README.md#未保存の文書とメモ用途)に記載した。
